...
{
    memset(list_, 0, DOMAIN_CHAR_COUNT * sizeof(char **) * DOMAIN_CHAR_COUNT);
...
int UrlFilter::prepare_buf(char *p, uint64_t size)
{
    ...
    int prefix_count[DOMAIN_CHAR_COUNT][DOMAIN_CHAR_COUNT] = {0};
    ...
    memcpy(this->list_count_, prefix_count, DOMAIN_CHAR_COUNT * DOMAIN_CHAR_COUNT * sizeof(int));
...
int SUrlFilter::load_url()
{
    ...
    pdqsort(url_list_, url_list_ + url_count_, compare_prefix_large);
    return 0;
}
...
void SUrlFilter::filter()
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        for (int j = 0; j < 2; ++j)
        {
            for (int k = 0; k < 2; ++k)
            {
                last_start[i][j][k] = pf_->pf_list_[i][j][k];
            }
        }
    }
    char **q_start = NULL;
    for (int i = 0; i < url_count_; ++i)
    {
        ...
        do
        {
            ...
            if (pf_->pf_count_[2][1][https] > 0)
            {
                ...
                q_start = last_start[2][1][https];
                end = pf_->pf_list_[2][1][https] + pf_->pf_count_[2][1][https];
                p_res = upper_bound(q_start, end, st, cmp_pf_large);
                if (p_res != end)
                {
                    pb = *p_res;
                    nb = *(uint16_t *)pb;
