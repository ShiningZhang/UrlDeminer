...
void ReadFile_Module::svc()
{
    ...
    for (SP_Message_Block_Base *msg = 0; get(msg) != -1;)
    {
        ...
        uint64_t size = 0;
        uint64_t begin = 0;
        uint64_t line_size = length < 1024 * 8 ? length : ceil((double)length / 16) + 64;
...
ReadFile_Module_v1::ReadFile_Module_v1(int threads)
    : threads_num_(threads)
{
}

ReadFile_Module_v1::~ReadFile_Module_v1()
{
}

int ReadFile_Module_v1::open()
{
    activate(threads_num_);
    return 0;
}

void ReadFile_Module_v1::svc()
{
    char *buf;
    Request *data = NULL;
    CRequest *c_data = NULL;
